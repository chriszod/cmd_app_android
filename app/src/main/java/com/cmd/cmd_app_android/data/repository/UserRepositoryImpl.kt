...
class UserRepositoryImpl constructor(
    ...
    override suspend fun loginUser(email: String, password: String): Flow<Resource<UserDTO>> = flow {
        emit(Resource.Loading())
        try {
                ...
                response.body()?.let {
                    if(it) {
                        ...
                        if(user.isSuccessful) {
                            user.body()?.let { userDto ->
                                emit(Resource.Success<UserDTO>(userDto))
                            }
                        } else {
    ...
    override suspend fun changePassword(userId: String, newPassword: String): Flow<Resource<UserDTO>> = flow {
        emit(Resource.Loading())
        try {
